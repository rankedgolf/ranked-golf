  ...
  const rankedEventRounds =
    eventRounds
      ?.map((round: any) => ({
        ...round,
        event_points:
          Number(round.points || 0) *
          (Number(round.holes || 18) === 9 ? 0.5 : 1),
      }))
      .sort(
        (a: any, b: any) =>
          Number(b.event_points) - Number(a.event_points) ||
          Number(b.trust_level || 0) - Number(a.trust_level || 0) ||
          Number(a.score_differential || 999) -
            Number(b.score_differential || 999)
      ) || [];
  ...
  return (
    <main className="min-h-screen p-8">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-4 rounded-xl border bg-green-50 p-4 text-sm text-green-800">
          <p className="font-semibold">Event Scoring</p>
          <p className="mt-1">
            18-hole rounds receive full event points. 9-hole rounds are eligible
            and receive 50% event points.
          </p>
        </div>

        {pageParams.error === "membership_required" && (
          <div className="mt-4 rounded-xl border bg-yellow-50 p-4 text-sm text-yellow-800">
            Prize events require a paid membership tier. Upgrade options are
            coming soon.
          </div>
        )}
          ...
          {existingRegistration ? (
            <div className="rounded-xl border bg-green-50 p-5 text-green-800">
              ...
              <p className="mt-2 text-sm">
                ...
                leaderboard, earn seasonal ranking points, and see how your game
        ...
        <section className="mt-8 rounded-xl border bg-yellow-50 p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-4">
            <div>
              ...
              <p className="text-xl font-semibold">{winningRound.score}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-xl font-semibold">
                {Number(winningRound.event_points).toFixed(2)}
              </p>
            </div>
          </div>
        </section>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="p-3">Holes</th>
                <th className="p-3">Diff</th>
                <th className="p-3">Event Points</th>
                <th className="p-3">Trust</th>
              </tr>
            </thead>

            <tbody>
              {rankedEventRounds?.map((round, index) => (
